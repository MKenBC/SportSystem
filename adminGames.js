...
gameID.textContent = localStorage.getItem("passEventID");

const holdEventID = localStorage.getItem("passEventID");
const holdSchoolYear = localStorage.getItem("eventCode");
...
function renderGames(content, ids) {
  ...
  btnShowDel.addEventListener("click", () => {
    ...
    txtDate.textContent = content.StartDate;
  ...
  btnViewGame.addEventListener("click", () => {
    localStorage.setItem("passGameID", ids);
    localStorage.setItem("gameName", content.GameName);
    localStorage.setItem("passDate", content.StartDate);
    if (content.TournamentType == "Round Robin") {
      window.location.href = "./adminLeagueRR.html";
